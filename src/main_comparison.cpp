...
int main() {
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "  Running Biot-Savart Model" << std::endl;
    std::cout << "========================================\n" << std::endl;
...
#pragma omp parallel reduction(+:biot_converged)
    {
        GenericPoseOptimizer biot_opt(&biot_adapter, config::optimizer::MAX_ITERATIONS, config::optimizer::TOLERANCE);
...
#pragma omp for schedule(dynamic, 4)
        for (int row = 0; row < total_rows; ++row) {
            ...
            apply_calibration(obs_data.sensor_positions[row], obs_data.B_measured[row], cal_data, sens_cal, B_cal);

            auto start = std::chrono::high_resolution_clock::now();
            PoseResult res = biot_opt.estimate_pose(sens_cal, B_cal, p_ref, u_ref, 1.0);
            ...
            biot_status[row] = res.converged ? res.status : 0;
            if (res.converged) biot_converged++;

            biot_pos_errors[row] = (res.position - p_ref).norm();
            double cos_angle = res.direction.dot(u_ref) / (res.direction.norm() * u_ref.norm());
            cos_angle = (std::max)(-1.0, (std::min)(1.0, cos_angle));
            biot_dir_errors[row] = std::acos(cos_angle) * 180.0 / M_PI;
    ...
    std::cout << "\n========================================" << std::endl;
    std::cout << "  Running Dipole Model" << std::endl;
    std::cout << "========================================\n" << std::endl;
...
#pragma omp parallel reduction(+:dipole_converged)
    {
        GenericPoseOptimizer dipole_opt(&dipole_adapter, config::optimizer::MAX_ITERATIONS, config::optimizer::TOLERANCE);
...
#pragma omp for schedule(dynamic, 4)
        for (int row = 0; row < total_rows; ++row) {
            ...
            auto start = std::chrono::high_resolution_clock::now();
            PoseResult res = dipole_opt.estimate_pose(sens_cal, B_cal, p_ref, u_ref, 1.0);
            auto end = std::chrono::high_resolution_clock::now();
            double elapsed_ms = std::chrono::duration<double, std::milli>(end - start).count();
            ...
            dipole_status[row] = res.converged ? res.status : 0;
            if (res.converged) dipole_converged++;

            dipole_pos_errors[row] = (res.position - p_ref).norm();
            double cos_angle = res.direction.dot(u_ref) / (res.direction.norm() * u_ref.norm());
            cos_angle = (std::max)(-1.0, (std::min)(1.0, cos_angle));
            dipole_dir_errors[row] = std::acos(cos_angle) * 180.0 / M_PI;
    ...
    save_detailed_results("results/biot_savart_results.csv", biot_row_indices, biot_positions,
        biot_directions, biot_scales, biot_rmse, biot_status,
        biot_pos_errors, biot_dir_errors, biot_time_ms);

    save_detailed_results("results/dipole_results.csv", dipole_row_indices, dipole_positions,
        dipole_directions, dipole_scales, dipole_rmse, dipole_status,
        dipole_pos_errors, dipole_dir_errors, dipole_time_ms);

    double biot_mean_pos = 0, biot_mean_dir = 0, biot_max_pos = 0, biot_max_dir = 0;
    double biot_mean_rmse = 0, biot_mean_time = 0;
    ...
    double dipole_mean_pos = 0, dipole_mean_dir = 0, dipole_max_pos = 0, dipole_max_dir = 0;
    double dipole_mean_rmse = 0, dipole_mean_time = 0;
    for (int i = 0; i < total_rows; ++i) {
    ...
    std::cout << "  Time: " << biot_duration.count() << " s" << std::endl;
    std::cout << "  Avg time: " << std::setprecision(2) << biot_mean_time << " ms" << std::endl;
    std::cout << "  RMSE: " << std::scientific << std::setprecision(2) << biot_mean_rmse
        << " T (" << (biot_mean_rmse * 1e6) << " uT)" << std::endl;
    ...
    std::cout << "  Time: " << dipole_duration.count() << " s" << std::endl;
    std::cout << "  Avg time: " << std::setprecision(2) << dipole_mean_time << " ms" << std::endl;
    std::cout << "  RMSE: " << std::scientific << std::setprecision(2) << dipole_mean_rmse
        << " T (" << (dipole_mean_rmse * 1e6) << " uT)" << std::endl;
    ...
    if (summary.is_open()) {
        ...
        summary << "  RMSE: " << dipole_mean_rmse << " T\n";
