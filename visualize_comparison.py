...
该脚本可视化Biot-Savart和Dipole模型的对比结果。
"""
...
def plot_comparison(biot_df, dipole_df, output_file="model_comparison.png"):
    ...
    ax.axvline(biot_df['rmse'].mean() * 1e6, color='blue', linestyle='--', linewidth=2,
               label=f'Biot Mean: {biot_df["rmse"].mean() * 1e6:.1f} µT')
    ax.axvline(dipole_df['rmse'].mean() * 1e6, color='red', linestyle='--', linewidth=2,
               label=f'Dipole Mean: {dipole_df["rmse"].mean() * 1e6:.1f} µT')
    ...
    bp = ax.boxplot(data_to_plot, labels=['Biot-Savart', 'Dipole'], patch_artist=True)
    ...
    stats = {
        'Metric': ['Mean RMSE (µT)', 'Mean Pos Error (mm)', 'Mean Dir Error (°)',
...
def plot_metrics_summary(biot_df, dipole_df, output_file="metrics_summary.png"):
    ...
    fig.suptitle('Key Metrics Summary\n关键指标总结', fontsize=16, fontweight='bold')

    # Calculate metrics
    metrics = {
        'RMSE (µT)': [biot_df['rmse'].mean() * 1e6, dipole_df['rmse'].mean() * 1e6],
        'Position Error (mm)': [biot_df['pos_error'].mean() * 1000, dipole_df['pos_error'].mean() * 1000],
        'Direction Error (°)': [biot_df['dir_error'].mean(), dipole_df['dir_error'].mean()],
...
def generate_report(biot_df, dipole_df, output_file="comparison_report.txt"):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"Mean RMSE:             {biot_df['rmse'].mean() * 1e6:.3f} µT\n")
        f.write(f"Median RMSE:           {biot_df['rmse'].median() * 1e6:.3f} µT\n")
        f.write(f"Std RMSE:              {biot_df['rmse'].std() * 1e6:.3f} µT\n")
        f.write(f"Mean Position Error:   {biot_df['pos_error'].mean() * 1000:.3f} mm\n")
        f.write(f"Mean Direction Error:  {biot_df['dir_error'].mean():.3f}°\n")
        ...
        f.write(f"Mean RMSE:             {dipole_df['rmse'].mean() * 1e6:.3f} µT\n")
        f.write(f"Median RMSE:           {dipole_df['rmse'].median() * 1e6:.3f} µT\n")
        f.write(f"Std RMSE:              {dipole_df['rmse'].std() * 1e6:.3f} µT\n")
        f.write(f"Mean Position Error:   {dipole_df['pos_error'].mean() * 1000:.3f} mm\n")
        f.write(f"Mean Direction Error:  {dipole_df['dir_error'].mean():.3f}°\n")
        ...
        pos_diff = ((biot_df['pos_error'].mean() - dipole_df['pos_error'].mean()) / dipole_df['pos_error'].mean()) * 100
        ...
        f.write(f"Position Error Diff:   {pos_diff:+.1f}% ({'Biot better' if pos_diff < 0 else 'Dipole better'})\n")
        f.write(f"Computation Time Diff: {time_diff:+.1f}% ({'Biot faster' if time_diff < 0 else 'Dipole faster'})\n\n")
