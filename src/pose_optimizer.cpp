...
namespace biot {
    ...
    void PoseOptimizer::compute_residuals_and_jacobian(
        ...
        // Analytical position, direction, and scale gradients
        ...
        for (size_t i = 0; i < n_sensors; ++i) {
            ...
            // Block 1: Position Gradient (Analytical)
            jacobian.block<3, 3>(i * 3, 0) = results[i].dB_dp;

            // Block 2: Direction Gradient (Analytical G_rot, used as ∂B/∂u_raw)
            jacobian.block<3, 3>(i * 3, 3) = results[i].dB_du;

            // Block 3: Scale Gradient (Analytical)
            jacobian.block<3, 1>(i * 3, 6) = results[i].dB_dscale;
        }

        // 原始的数值方向和尺度梯度计算代码已被移除，因为现在使用解析梯度。
        // 原本的代码块如下（已被移除）：
        /*
