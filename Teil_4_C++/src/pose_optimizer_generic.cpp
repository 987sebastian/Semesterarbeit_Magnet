...
namespace generic {
    ...
    PoseResult GenericPoseOptimizer::estimate_pose(
        ...
        const double practical_tolerance = 25.0e-6;
        const double minimal_rmse_for_minimal_improvement_check = 3.0e-5;
        ...
        for (int iter = 0; iter < max_iterations_; ++iter) {
            ...
            if (iter >= 6) {
                ...
                // Status 4: Stop if no improvement for 3 iterations
        ...
        if (!result.converged && best_rmse < 5e-6) {
            result.converged = true;
            result.status = 5;
        }
